...
namespace OOServerUS
{
    public class Yahoo
    {
        ...
        {
            string yahoo_ticker = CorrectSymbol(ticker);
            ...
            // body\div(3)\div(4)\br\table\tr(2)\td\table(3)\tr\td\table\tr(2)\td(3)\b\span

            XmlNode nd, table_nd = prs.GetXmlNodeByPath(main_xml.FirstChild, @"body\div(3)\div(4)\br\table");
            ...
            try
            {
                foreach (string url in url_list)
                {
                    ...
                    {
                        xml = DownloadXmlPartialWebPage(url, "<body", "</body>", 1, 1);
                    ...
                    for (int i = 1; ; i++)
                    {
                        table_nd = prs.FindXmlNodeByName(xml.FirstChild, "th", "class=yfnc_tablehead1", i);
                    ...
                    for (int i = 0; i < table_nd_list.Count; i++)
                    {
                        ...
                        for (int j = 2; ; j++)
                        {
                            try
                            {
                                ...
                                int gx;
                                int gc = symbol.LastIndexOf('C');
                                int gp = symbol.LastIndexOf('P');
                                ...
                                option.expiration = new DateTime(int.Parse(symbol.Substring(gx, 2)) + 2000, int.Parse(symbol.Substring(gx + 2, 2)), int.Parse(symbol.Substring(gx + 4, 2))).Date;
        ...
        public Quote GetQuote(string ticker)
        {
            ...
            path_list.Clear();
            LocateTag(xml.FirstChild, "h2" , path_list);
            if (path_list.Count > 0)
            {
                nd = prs.GetXmlNodeByPath(xml.FirstChild, path_list[3].Replace(@"\html\", ""));
            }
            else nd = null;                       
            ...
            path_list.Clear();
            LocateText(xml.FirstChild, "Open:" , path_list);
            if (path_list.Count > 0)
            {
                table = prs.GetXmlNodeByPath(xml.FirstChild, path_list[0].Replace(@"\html\", ""));
            ...
            // body\div\div(2)\div(3)\div(2)\div\div(2)\table(2)\tr(2)\td
            ...
            if (ticker.Contains("^"))
            {
                ...
                {
                    nd = prs.GetXmlNodeByPath(table, @"table(2)\tr(1)\" + colm + @"\span");
        ...
        public ArrayList GetHistoricalData(string ticker, DateTime start, DateTime end)
        {
            ...
            // create url ticker
            string yahoo_ticker;
            if (is_index || yahoo_exchange_suffix == "") yahoo_ticker = CorrectSymbol(ticker);
            else yahoo_ticker = CorrectSymbol(ticker) + "." + yahoo_exchange_suffix;
        ...
        private string getNodePath_rec(XmlNode nd_root)
        {
            if (nd_root.ParentNode == null) return "";
            else
            {
                int i = 0;
                foreach (XmlNode nd_child in nd_root.ParentNode.ChildNodes)
                {
                    if (nd_child.Name == nd_root.Name) i++;
                    if (nd_child == nd_root) break;
                }

                if (i == 1) return getNodePath_rec(nd_root.ParentNode) + @"\" + nd_root.Name;
                else return getNodePath_rec(nd_root.ParentNode) + @"\" + nd_root.Name + @"(" + i.ToString() + @")";
            }
        }

        private void LocateText(XmlNode nd_root, string text, List<string> path_list)
        {
            if (nd_root.ChildNodes.Count == 0 && nd_root.InnerText != null && nd_root.InnerText.Contains(text))
            {
                path_list.Add(getNodePath_rec(nd_root));
            }
            else
            {
                foreach (XmlNode nd_child in nd_root.ChildNodes)
                {
                    if (nd_child.InnerText != null && nd_child.InnerText.Contains(text)) LocateText(nd_child, text, path_list);
                }
            }
        }

        private void LocateTag(XmlNode nd_root, string tag, List<string> path_list)
        {
            if (nd_root.Name == tag)
            {
                path_list.Add(getNodePath_rec(nd_root));
            ...
            else
            {
                foreach (XmlNode nd_child in nd_root.ChildNodes)
                {
                    if (nd_child.InnerText != null) LocateTag(nd_child, tag, path_list);
                }
            }
        }
    }
